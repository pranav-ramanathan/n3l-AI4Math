...
\label{eq:sciml_flow}
\end{equation}
integrated with a fixed-step RK4 scheme on the GPU. After integration, an optional refinement stage applies ADAM followed by L-BFGS to polish solutions whose double-well energy has not fully collapsed. The horizon $T$ is grid-size dependent (Section 3.5).

\subsubsection{Parallel Trajectory Search on Apple Metal}

We implement Equation~\eqref{eq:sciml_flow} as a custom Metal compute kernel (\texttt{main\_metal\_v3.jl}) that integrates $R$ independent trajectories in parallel, one per GPU thread, with no inter-thread communication~\cite{Metal_jl}. Each thread holds private scratch for an $n \times n$ board (capped at $n \le 32$ by kernel-local memory) and reads a precomputed sparse line table that encodes the constraint structure once per run. We launch $R = 30$ trajectories per run by default. After integration, each trajectory is converted to a binary configuration by retaining the top $2n$ entries of $\mathbf{x}(T)$ above a threshold $\tau$, and the best valid configuration found along each trajectory is recorded. Most successful runs reach their final placement within the first 10--20\% of the integration horizon, motivating an early-best heuristic that returns once any thread reports a zero-violation $2n$-point configuration.
...
% zero violations, so the cell is "--" (best 33 / 1 viol and 35 / 3
% viol respectively, discussed in the SciML paragraph below).
\resizebox{\columnwidth}{!}{%
...
Figure \ref{fig:patternboost_loss} shows PatternBoost's loss over 15,000 training steps. Both training and test loss drop from above 5.0 to near 0.1, with a minimal train-test gap throughout, indicating strong generalization without overfitting.
