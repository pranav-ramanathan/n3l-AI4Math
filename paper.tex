...
\begin{itemize}
    \item \textbf{A comparative study.} The first systematic comparison of four search paradigms: ILP, learned pattern search, reinforcement learning, and SciML continuous relaxation, studying where each paradigm succeeds and how it fails.
    ...
    \item \textbf{PatternBoost on collinearity.} The first application of PatternBoost \cite{Charton2024} to the No-Three-In-Line problem.
    \item \textbf{PPO with action masking.} The first application of a PPO reinforcement-learning agent \cite{Schulman2017} with action masking.
\end{itemize}
...
Our study presents a comparison of classical, learning-based, and SciML approaches to the No-Three-In-Line problem. Integer linear programming performed best on the grids it could enumerate, finding optimal solutions for up to $n = 19$ and demonstrating the power of classical solvers for constraint satisfaction problems. In contrast, reinforcement learning approaches achieved perfect solutions only up to $n = 10$, with performance degrading by 42\% at $n = 11$ due to constraint violations. PatternBoost, the hybrid method combining classical greedy algorithms with transformers, achieved optimal solutions up to $n = 14$ and modest generalization to $n = 15$, bridging the gap between pure reinforcement learning and classical optimization. The continuous-relaxation SciML method extended this further, recovering optimal $2n$ configurations on Apple Metal up to $n = 18$ without exploiting symmetry --- well beyond the PatternBoost ceiling of $n = 14$ and the PPO ceiling of $n = 10$. These findings underscore that reformulating discrete combinatorial problems in continuous form, and that hybrid approaches combining algorithmic heuristics with learned pattern recognition, both offer promising paths to scaling beyond exact-search regimes.
...
This study is limited to grid sizes up to $n = 19$ for ILP, $n = 15$ for PatternBoost, $n = 11$ for PPO, and $n = 19$ for SciML, preventing definitive conclusions about asymptotic scaling behavior. We did not explore hybrid training combining supervised and reinforcement learning, transfer learning across grid sizes, or curriculum learning strategies that could leverage synergies between approaches. Computational constraints prevented exhaustive hyperparameter tuning and investigation of architectural improvements such as multi-branch networks or enhanced attention mechanisms for capturing long-range geometric dependencies. The PatternBoost model relies heavily on greedy-generated training data quality, with greedy algorithm limitations potentially propagating to the learned model. PPO's reward structure remains relatively simple and may benefit from more sophisticated shaping, intrinsic motivation signals, or exploration bonuses to better handle sparse binary rewards in constraint satisfaction tasks. The SciML pipeline was evaluated only on Apple Metal: ROCm and CUDA backends were not cross-validated, the per-grid hyperparameter schedule was tuned manually with a single run configuration per $n$, and we did not exploit the rotational and reflective symmetries that the classical solvers rely on at larger $n$.

Future research should prioritize hybrid training paradigms that combine supervised pretraining (PatternBoost's cross-entropy learning) with reinforcement fine-tuning (PPO's exploratory optimization), mirroring successful approaches in language modeling. Transfer learning across grid sizes and curriculum strategies could enable training on smaller grids before scaling to larger instances, with meta-learning potentially discovering optimization strategies that generalize across problem scales. Classical-AI integration offers additional promising directions: ILP warm-starting with PatternBoost candidates could accelerate convergence on larger grids, while PatternBoost training on ILP-optimal smaller solutions could improve pattern quality beyond greedy baselines. Our SciML method is itself a concrete instantiation of the physics-inspired direction advocated by~\cite{schuetz2022combinatorial}: reformulating combinatorial problems as energy minimisation over a relaxed Hamiltonian, then solving with gradient-based methods. Extending this beyond the No-Three-In-Line problem --- in particular to other forbidden-configuration problems in combinatorial geometry --- is a natural next step.
...
The code for all four methods is available in the following repositories:
