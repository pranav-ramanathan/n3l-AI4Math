...
ILP achieves provably optimal solutions up to $19 \times 19$ grids. PatternBoost matches optimal performance up to $14 \times 14$ grids and finds 29 points on $15 \times 15$ grids (optimal is 30). PPO achieves perfect solutions on $10 \times 10$ grids but fails at $11 \times 11$ due to constraint violations. SciML recovers optimal $2n$ configurations for every $n$ from $10$ to $18$ except $n = 17$, on commodity GPU hardware and without symmetry pruning.
...
\begin{equation}
E(\mathbf{x}) = \alpha \, E_{\text{coll}}(\mathbf{x}) + E_{\text{count}}(\mathbf{x}) + \gamma \, E_{\text{bin}}(\mathbf{x}),
\label{eq:sciml_energy}
\end{equation}
where $E_{\text{coll}}$ penalises every grid line by the smooth excess of its summed mass over two, $E_{\text{count}} = -\sum_{i,j} x_{i,j}$ rewards point count, and $E_{\text{bin}} = \sum_{i,j} 4 x_{i,j}^2 (1 - x_{i,j})^2$ is a double-well term that pushes each $x_{i,j}$ toward $\{0,1\}$. The collinearity term reuses the line-incidence structure enumerated in Section 2.1, replacing the $O(n^6)$ triple enumeration with $O(n^3)$ per-line sums. Per-line penalties are normalised by line length (\textit{mean-incidence}), which keeps $\alpha$ within a narrow range as $n$ grows.
...
Binary regularisation $\gamma$ & $4.0$--$5.0$ \\
...
Refinement & optional ADAM $\rightarrow$ L-BFGS \\
\midrule
\multicolumn{2}{l}{\textup{Search}} \\
Parallel trajectories $R$ & $30$ \\
...
\end{center}
\vspace{0.1in}
{\footnotesize \textit{Note:} {--} indicates either a grid size not tested for that method or a run that did not yield a valid zero-violation configuration.}
\end{table}
...
\textbf{PatternBoost.} PatternBoost achieved optimal solutions up to $n = 14$ (28 points) and discovered 29-point configurations on $n = 15$ grids, one short of the known optimum, but did not scale beyond $n = 15$.

\textbf{Proximal Policy Optimization.} PPO achieved perfect solutions on $n = 10$ grids (20 points, zero violations) but incurred a collinearity violation on $n = 11$ grids, revealing difficulty maintaining constraint satisfaction as problem complexity increases.

\textbf{SciML.} The gradient flow recovered optimal $2n$-point configurations for every $n \in \{10,\ldots,18\} \setminus \{17\}$, with per-run success rates from $4/10$ at $n=14$ to $10/10$ at $n=10$ and $n=12$. The best $n=18$ run reached 36 points in ${\sim}588$\,s on Apple M2. Two failure modes are visible: at $n=17$ one seed produced a valid 34-point configuration, but the 10-run ablation yielded 0/10 successes (best: 34 points, 0 violations on a single seed; systematic runs: 0/10, min 2 residual violations); at $n=19$ the best attempt reached 35 points with three violations.
