...
\textbf{Parameter} & \textbf{Value} \\
\midrule
\multicolumn{2}{l}{\textup{Energy}} \\
Collinearity penalty $\alpha$ & per-$n$ schedule (Sec.~3.5) \\
Binary regularisation $\mu$ & $4.0$--$5.0$ \\
Per-line normalisation & mean-incidence \\
\midrule
\multicolumn{2}{l}{\textup{Solver}} \\
Integrator & fixed-step RK4 \\
Time horizon $T$ & $10$ ($n \le 15$), $25$ ($n=16$), $30$ ($n \ge 17$) \\
Post-processing & top-$2n$ thresholding only \\
\midrule
\multicolumn{2}{l}{\textup{Search}} \\
Parallel trajectories $R$ & $5000$ \\
Initialisation & uniform random in $[0,1]^{n^2}$ \\
Threshold $\tau$ & top-$2n$ entries of $\mathbf{x}(T)$ \\
...
Our study presents a comparison of classical, learning-based, and SciML approaches to the No-Three-In-Line problem. These findings underscore that reformulating discrete combinatorial problems in continuous form, and that hybrid approaches combining algorithmic heuristics with learned pattern recognition, both offer promising paths to scaling beyond exact-search regimes.
