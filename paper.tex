...
\begin{abstract}
% Pre-SciML abstract preserved below for reference. Re-enable by
% removing the \iffalse..\fi guard.
\iffalse
...
\midrule
$5\times 5$   & 10 & 10 & 10 & -- & 10 \\
$8\times 8$   & 16 & 16 & 16 & -- & 16 \\
$10\times 10$ & 20 & 20 & 20 & 20 (10/10) & 20 \\
$11\times 11$ & -- & -- & -- & 22 (7/10)  & 22 \\
$12\times 12$ & 24 & 24 & -- & 24 (10/10) & 24 \\
$13\times 13$ & -- & -- & -- & 26 (8/10)  & 26 \\
$14\times 14$ & 28 & 28 & -- & 28 (4/10)  & 28 \\
$15\times 15$ & 30 & 29 & -- & 30 (8/10)  & 30 \\
$16\times 16$ & -- & -- & -- & 32 (8/10)  & 32 \\
$17\times 17$ & -- & -- & -- & 34*        & 34 \\
$18\times 18$ & -- & -- & -- & 36 (9/10)  & 36 \\
$19\times 19$ & 38 & -- & -- & --$^{\dagger}$ & 38 \\
\bottomrule
\end{tabular}
\end{sc}
\end{footnotesize}
\end{center}
\vspace{0.1in}
{\footnotesize \textit{Note:} {--} indicates either a grid size not tested for that method or a run that did not yield a valid zero-violation configuration. * reproduced for one seed; original 10-run sweep: 0/10. $^{\dagger}$ SciML at $19\times19$: attempted 10-run sweep, 0/10 valid solutions; best attempt reached 35 points with 3 violations.}
\end{table*}
...
% Pre-SciML version of Discussion (4 paragraphs) preserved below for
% reference. Re-enable by removing the \iffalse..\fi guard.
\iffalse
...
% Current 4-paragraph version preserved below; pre-SciML version in \iffalse block above.
\iffalse
...
The distinct failure modes observed across methods reveal fundamental trade-offs in combinatorial optimization. ILP's branch-and-bound approach guarantees optimality through systematic search space pruning but faces exponential growth in problem complexity, illustrating the combinatorial explosion inherent to exact optimization. PatternBoost's philosophy of rapid training data generation through greedy algorithms creates a quality-speed trade-off: the greedy approach struggles to find high-quality placements as problem complexity increases, constraining the quality of learned patterns. Reinforcement learning improved over training iterations but consistently produced constraint violations beyond $n = 10$, revealing fundamental difficulty in satisfying geometric constraints through reward-based exploration alone. The SciML method exhibits a different failure mode: a continuous-discrete gap that opens at $n = 17$ and $n = 19$, where the gradient flow stabilises configurations with one to three residual collinearities and the threshold $\tau$ cannot reliably round them to a valid binary solution; this regime is also where raising $\alpha$ further tends to freeze trajectories in bad local minima before convergence. These computational bottlenecks underscore that each method occupies a distinct niche in the optimization landscape.

Each method was evaluated up to the grid size at which it could no longer reliably produce valid solutions, with computational cost as the primary limiting factor for ILP. Unlike SciML, whose shorter run times made 10-seed sweeps feasible on the M2 MacBook, PPO and PatternBoost were evaluated from a single training run because repeating their much longer end-to-end training across seeds was not practical under our hardware budget. Wall-clock times are therefore not directly comparable across methods. Computational constraints prevented exhaustive hyperparameter tuning and investigation of architectural improvements. The PatternBoost model relies heavily on greedy-generated training data quality, with greedy algorithm limitations potentially propagating to the learned model. PPO's reward structure remains relatively simple, limiting its ability to handle sparse binary rewards in constraint satisfaction tasks. The SciML pipeline was evaluated only on Apple Metal: ROCm and CUDA backends were not cross-validated, the per-grid hyperparameter schedule was tuned manually with a single run configuration per $n$, and we did not exploit the rotational and reflective symmetries that the classical solvers rely on at larger $n$.

Future research should prioritize hybrid training paradigms that combine supervised pretraining (PatternBoost's cross-entropy learning) with reinforcement fine-tuning (PPO's exploratory optimization), mirroring successful approaches in language modeling. Transfer learning across grid sizes and curriculum strategies could enable training on smaller grids before scaling to larger instances, with meta-learning potentially discovering optimization strategies that generalize across problem scales. Classical-AI integration offers additional promising directions: ILP warm-starting with PatternBoost candidates could accelerate convergence on larger grids, while PatternBoost training on ILP-optimal smaller solutions could improve pattern quality beyond greedy baselines. Extending the SciML approach beyond the No-Three-In-Line problem --- in particular to other forbidden-configuration problems in combinatorial geometry --- is a natural next step.
\fi
