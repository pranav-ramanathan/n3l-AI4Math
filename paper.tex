...
The collinearity-penalty schedule has two regimes. For $n \le 10$ we use unnormalised line penalties and $\alpha$ rises linearly with $n$, from $\alpha = 3.3$ at $n = 2$ to $\alpha = 16.7$ at $n = 10$ --- a slope of roughly $5/3$ per unit grid size. From $n = 11$ onwards we switch to mean-incidence normalisation (Section~2.5.1), which divides each line's penalty by line length; $\alpha$ then no longer needs to grow with the constraint count, and we hold it near $10$ for $n = 11$--$16$, decreasing to $7$ at $n = 18$. The integration horizon $T$ stays at $10$ for $n \le 15$, $25$ at $n = 16$, and $30$ for $n = 17,18$. The reported $n=19$ exploratory sweep used a separate $T=10$, $\alpha=8$ setting. The two regimes where success collapses, $n = 17$ and $n = 19$, coincide with the schedule's tightest margin: the unnormalised regime would have required $\alpha \approx 30$ at $n = 17$, beyond what the GPU's Float32 dynamic range can stably support.
...
\begin{figure}[htbp]
    ...
    \caption{SciML collinearity-penalty schedule $\alpha(n)$ (blue, left axis) and integration horizon $T(n)$ (orange, right axis). Shaded regions mark the unnormalised ($n \le 10$) and mean-incidence ($n \ge 11$) regimes.}
